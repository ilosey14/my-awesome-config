...
local editor_cmd = string.format('%s -e %s', terminal, os.getenv('EDITOR') or 'nano')
...
-- Expecting a wm_name if awesome omits too many applications and tools
menu_utils.wm_name = ''
...
menu.has_value = function (tab, value)
...
local screenshot_menu = {
	{
		...
		function ()
			timers.set_timeout(
				function () awful.spawn.easy_async_with_shell(apps.full_screenshot) end,
				0.1)
		end,
	...
	},
	{
		...
		function ()
			timers.set_timeout(
				function () awful.spawn.easy_async_with_shell(apps.area_screenshot) end,
				0.1)
		end,
		menubar.utils.lookup_icon('accessories-screenshot')
	}
